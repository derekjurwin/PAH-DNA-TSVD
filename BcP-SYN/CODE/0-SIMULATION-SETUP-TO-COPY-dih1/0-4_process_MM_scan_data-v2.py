...
#print(sorted_QM_angles_list)
#print ('\n')
#print(sorted_MM_relative_energies_list)
#print ('\n')
#print(QM_relative_energies_by_angle)
    
out_to_file=open('0_12_MM_plot_data.txt','w')
...
out_to_file=open('0_12_QM_plot_data.txt','w')
for i in range(len(sorted_QM_angles_list)):
    out_to_file.write(str(sorted_QM_angles_list[i]))
    out_to_file.write(',')
...
out_to_file1=open('0_10_matlab_sorted_angles_data.txt','w')
out_to_file2=open('0_11_matlab_diff_pot_data.txt','w')
...
#    out_to_file.write('Scan point '+str(a+1)+' dihedral angle: '+str(np.degrees(np.arctan2(y,x)))+' QM(dih)-MM(dih) = '+str(np.degrees(np.arctan2(y,x))-angles_ordered_by_scan_point_list[a]))
    out_to_file.write('Scan point '+str(a+1)+' dihedral angle: '+str(np.degrees(np.arctan2(y,x)))+' QM(dih)-MM(dih) = '+str(np.degrees(np.arctan2(y,x))-angles_ordered_by_scan_point_QM_MM_check_list[a]))
    if abs(np.degrees(np.arctan2(y,x))-angles_ordered_by_scan_point_QM_MM_check_list[a]) > 0.1:
        out_to_file.write('*******************')
    out_to_file.write('\n')
